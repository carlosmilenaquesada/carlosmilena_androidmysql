...
public class AgregarNuevoJuegoActivity extends AppCompatActivity{
	...
	public void insertarJuegoConFoto(View view){
		...
		if(spNuevoPlataforma.getSelectedItemPosition() == -1 || (spNuevoGenero.getSelectedItemPosition() == -1)){
			Toast.makeText(AgregarNuevoJuegoActivity.this, "No pudo cargarse la lista de Géneros y Plataformas. Cambios no guardados.",
			...
			public void onResponse(String response){
				if(response.equalsIgnoreCase("datos insertados")){
					...
					if(imagen_seleccionada != null){
						insertarFotodb(juegoNuevo.getIdentificador(), ivNuevoImagen);
					}
	...
	private void insertarFotodb(String idimagen, ImageView ivNuevoImagen){
		StringRequest request = new StringRequest(Request.Method.POST,
				ConfiguracionDB.DIRECCION_URL_RAIZ +
