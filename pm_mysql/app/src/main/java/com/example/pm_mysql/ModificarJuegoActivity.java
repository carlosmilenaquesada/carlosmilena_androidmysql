...
import com.example.pm_mysql.recyclerview.JuegoViewHolder;
import com.example.pm_mysql.utilidades.Herramientas;
import com.example.pm_mysql.utilidades.ImagenesBlobBitmap;
...
public class ModificarJuegoActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
		...
		StringRequest request = new StringRequest(Request.Method.POST,
			...
			public void onResponse(String response){
				if(response.equalsIgnoreCase("datos eliminados")){
					...
					borrarFotodb(juego.getIdentificador());
	...
	private void borrarFotodb(String idimagen){
		StringRequest request = new StringRequest(Request.Method.POST,
				ConfiguracionDB.DIRECCION_URL_RAIZ +
	...
	private void insertarFotodb(String idimagen, ImageView nuevaImagen){
		StringRequest request = new StringRequest(Request.Method.POST,
				ConfiguracionDB.DIRECCION_URL_RAIZ +
	...
	public void editarJuego(View view){
		...
		if(spModificarJuegoPlataforma.getSelectedItemPosition() == -1 || (spModificarGenero.getSelectedItemPosition() == -1)){
			Toast.makeText(ModificarJuegoActivity.this, "No pudo cargarse la lista de Géneros y Plataformas. Cambios no guardados.",
					Toast.LENGTH_LONG).show();
			return;
		}
				...
				if(response.equalsIgnoreCase("datos actualizados")){
					if(imagenSeleccionada != null ||
					   !juego.getIdentificador().equals(juegoModificado.getIdentificador())){
						borrarFotodb(juego.getIdentificador());
						insertarFotodb(juegoModificado.getIdentificador(), ivModificarImagen);
					}
